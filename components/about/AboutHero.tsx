...
const metrics = [
    { icon: facebookIcon, alt: 'Facebook', num: 475, suffix: 'K+', label: 'FB Followers', decimal: false },
    { icon: instagramIcon, alt: 'Instagram', num: 50, suffix: 'K+', label: 'IG Followers', decimal: false },
    { icon: tiktokIcon, alt: 'TikTok', num: 80, suffix: 'K+', label: 'TikTok Fans', decimal: false },
    { icon: youtubeIcon, alt: 'YouTube', num: 12, suffix: 'K+', label: 'Subscribers', decimal: false },
    { icon: null, alt: 'Rating', num: 4.8, suffix: '', label: 'Google Rating', decimal: true },
    { icon: null, alt: 'Customers', num: 100, suffix: 'K+', label: 'Customers', decimal: false },
];
        ...
        const observer = new IntersectionObserver(([entry]) => {
            ...
            const tick = (now: number) => {
                ...
                setCounts(metrics.map(m =>
                    m.decimal
                        ? parseFloat((m.num * eased).toFixed(1))
                        : Math.round(m.num * eased)
                ));
                    ...
                    <div ref={metricsRef} className="w-full pt-8 border-t border-slate-100">
                        <div className="flex flex-wrap justify-center gap-4 sm:gap-6 w-full">
                            {metrics.map((item, index) => (
                                <div
                                    key={index}
                                    className="w-[130px] sm:w-37.5 flex flex-col items-center justify-center gap-2 p-3 sm:p-4 bg-white border border-slate-200 rounded-xl shadow-xs hover:border-slate-300 hover:shadow-sm transition-all"
                                >
                                    {item.icon ? (
