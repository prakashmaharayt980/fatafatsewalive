...
export default function HeaderActionsClient({ initialIsLoggedIn }: Props) {
    ...
    const authResolved = hasHydrated ? isLoggedIn : initialIsLoggedIn;
                ...
                <>
                    <div className="relative">
                        <button
                            ...
                            className="hidden sm:flex items-center space-x-1 p-1.5 rounded-full border border-gray-300 text-slate-700 hover:text-[var(--colour-fsP2)] hover:border-[var(--colour-fsP2)] hover:bg-blue-50 transition-all font-bold"
                        >
                        ...
                        {showAccountMenu && (
                            <div className="absolute right-0 top-full mt-2 w-60 bg-white border border-gray-200 rounded-lg shadow-lg z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                                <div className="px-4 py-3 border-b border-gray-100 font-bold">
                                    <p className="text-sm font-semibold text-gray-900 truncate tracking-tight">{user?.name || 'Account'}</p>
                                ...
                                <div className="py-1">
                                    {[
                                        ...
                                        <Link
                                            ...
                                            className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors font-bold"
                                        >
                                            <item.icon className="w-4 h-4 text-gray-500" />
                                            {item.label}
                                        </Link>
                                    ))}
                                    <div className="h-px bg-gray-100 my-1 font-bold" />
                                    <button
