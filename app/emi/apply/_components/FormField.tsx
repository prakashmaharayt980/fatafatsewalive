...
const FormField: React.FC<FormFieldProps> = React.memo(({ field, error }) => (
    <div className={`${field.extenduserinfo || ''} m-0`}>
        <label className="text-[10px] font-bold text-gray-600 uppercase tracking-wider block mb-0.5">{field.label}</label>
        <div className="relative group">
            ...
            {field.type === 'select' ? (
                ...
                >
                    <SelectTrigger className={`w-full h-9 pl-10 bg-white text-gray-700 text-sm rounded-lg border transition-all duration-150 ${error ? 'border-red-400 ring-1 ring-red-200' : 'border-gray-200 focus:border-(--colour-fsP2) focus:ring-1 focus:ring-(--colour-fsP2)'} ${field.disabled ? 'opacity-60 cursor-not-allowed bg-gray-50' : ''}`}>
                ...
                <Input
                    ...
                    disabled={field.disabled}
                    autoComplete="off"
                    className={`w-full h-9 pl-10 text-gray-700 text-sm rounded-lg border transition-all duration-150 ${error ? 'border-red-400 ring-1 ring-red-200 bg-red-50/30' : 'border-gray-200 bg-white focus:border-(--colour-fsP2) focus:ring-1 focus:ring-(--colour-fsP2)'} ${field.disabled ? 'opacity-60 cursor-not-allowed bg-gray-50' : ''}`}
                />
            )}
        </div>
