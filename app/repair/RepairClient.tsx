...
import {
    ...
    getRepairEstimate, getRepairLabels, initialRepairForm, type RepairFormData
} from './repair-helpers'
...
export default function RepairClient({ brands }: RepairClientProps) {
    ...
    // ── State ────────────────────────────────────────────────
    const { guestAddresses, addGuestAddress } = useCartStore(useShallow(state => ({
    ...
    const handleSelectProduct = (product: ProductListItem) => {
        const img = typeof product.image === 'string' ? product.image : product.image?.thumb || logoImg.src
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
                            ...
                            <div className="md:w-1/2 relative flex justify-center">
                                ...
                                <div className="relative z-10 w-full max-w-[350px] md:max-w-[400px] aspect-square">
                                    <Image src={logoImg} alt="Device Repair - Fatafat Sewa" fill className="object-contain drop-shadow-2xl" sizes="(max-width: 768px) 100vw, 50vw" priority fetchPriority="high" />
                                </div>
                            </div>
                        </div>
                    </div>
